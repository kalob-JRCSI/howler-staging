// HOWLER Scheduling Intelligence staging bundle v0.9.3
...
function computeConfidence(model, activity, generatedAt) {
    ...
    const eventSourceIds = new Set(model.eventLedger
        .filter((event) => event.impactSeedActivityIds.includes(activity.id))
        .flatMap((event) => event.sourceIds ?? []));
    const eventSources = [...eventSourceIds].map((id) => model.sources[id]).filter((source) => Boolean(source));
    const fieldEvidenceSources = [...sources, ...eventSources]
        .filter((source) => ["FIELD_REPORT", "ACTUAL_VERIFICATION"].includes(source.type));
    const verifiedConstraintFloor = constraints.some((constraint) => constraint.state === "SATISFIED" &&
        ["FIELD_VERIFIED", "VERIFIED_ACTUAL", "PM_CONFIRMED"].includes(constraint.verification ?? "")) ? 0.88 : 0.4;
    const fieldVerification = activity.actualFinish ? 1 : activity.actualStart ? 0.95 : average(fieldEvidenceSources.map(sourceScore), verifiedConstraintFloor);
...
function runOversightReview(model, candidate, triggeringEvent, createdAt) {
    ...
    if (criticalShifted.length > 0) {
        ...
        if (candidate.recoveryAnalysis?.recoveryAvailable) {
            addFinding(findings, "CRITICAL_PATH", "WARN", `Advisory recovery capacity detected: up to ${candidate.recoveryAnalysis.recoverableWorkdays} workday(s), with modeled completion ${candidate.recoveryAnalysis.recoverableCompletionLikely}. Recovery remains conditional until PM/trade/field evidence confirms the selected lever.`, candidate.recoveryAnalysis.criticalShiftActivityIds ?? []);
        }
    }
    else {
    ...
    for (const mutation of event.mutations) {
        switch (mutation.op) {
            ...
            case "SUPERSEDE_SOURCE": {
                ...
                if (!source)
                    throw new Error(`Unknown source in SUPERSEDE_SOURCE: ${mutation.sourceId}`);
                if (!sources[mutation.supersededBySourceId])
                    throw new Error(`Unknown superseding source in SUPERSEDE_SOURCE: ${mutation.supersededBySourceId}`);
...
function forecastBasisFor(model, activity) {
    if (activity.actualFinish || activity.actualStart || activity.state === "COMPLETE" || activity.state === "IN_PROGRESS") {
    ...
    for (const constraintId of activity.constraintIds) {
        ...
        assumptions.push(`${constraint.type}: ${constraint.label} [${constraint.state}]`);
    }
    const graph = (0, graph_js_1.buildGraphIndex)(model);
    for (const dep of graph.incoming[activity.id] ?? []) {
        if (!dep.hard)
    ...
    if (assumptions.length > 0)
        return { type: "CONDITIONAL", assumptions };
    if (hasCommitment)
        return { type: "COMMITMENT_SUPPORTED", assumptions: [] };
    return { type: "MODEL_FORECAST", assumptions: [] };
}
function buildForecastDelta(model, activityForecasts, completion, baseline) {
    if (!baseline)
        return undefined;
    const shiftedActivities = [];
    for (const current of Object.values(activityForecasts)) {
        const prior = baseline.activityForecasts?.[current.activityId];
        if (!prior)
            continue;
        const startDelta = (0, date_js_1.workdaysBetween)(prior.start.likely, current.start.likely, model.calendar);
        const finishDelta = (0, date_js_1.workdaysBetween)(prior.finish.likely, current.finish.likely, model.calendar);
        ...
        shiftedActivities.push({
            ...
            startLikely: { from: prior.start.likely, to: current.start.likely, deltaWorkdays: startDelta },
            finishLikely: { from: prior.finish.likely, to: current.finish.likely, deltaWorkdays: finishDelta },
        });
    }
    const completionLikelyDeltaWorkdays = (0, date_js_1.workdaysBetween)(baseline.completion.likely, completion.likely, model.calendar);
    return {
        fromSnapshotId: baseline.id,
        fromVersion: baseline.version,
        completionLikely: { from: baseline.completion.likely, to: completion.likely, deltaWorkdays: completionLikelyDeltaWorkdays },
        shiftedActivityCount: shiftedActivities.length,
        criticalShiftCount: shiftedActivities.filter((item) => item.critical).length,
        shiftedActivities,
    };
}
function supersededSourceSummary(model) {
    return Object.values(model.sources)
        .filter((source) => Boolean(source.supersededBySourceId))
        .map((source) => {
        const superseding = model.sources[source.supersededBySourceId];
        return {
            sourceId: source.id,
            label: source.label,
            effectiveDate: source.effectiveDate ?? null,
            supersededBySourceId: source.supersededBySourceId,
            supersedingLabel: superseding?.label ?? source.supersededBySourceId,
            supersedingObservedAt: superseding?.observedAt ?? null,
        };
    });
}
function analyzeRecovery(model, current, baseline) {
    if (!current) {
        return {
            status: "NO_FORECAST",
    ...
    const baselineCompletionLikely = baseline?.completion?.likely ?? delta?.completionLikely?.from ?? null;
    const currentCompletionLikely = current.completion?.likely ?? null;
    if (delayWorkdays <= 0) {
        return {
            status: "ON_TRACK",
            recoveryAvailable: false,
    ...
    const graph = (0, graph_js_1.buildGraphIndex)(model);
    const shiftedCritical = (delta?.shiftedActivities ?? []).filter((item) => item.critical);
    const shiftedCriticalIds = new Set(shiftedCritical.map((item) => item.activityId));
    const levers = [];
    const protectionActions = [];
    for (const dep of Object.values(model.dependencies)) {
        if (!dep.active || !dep.hard || dep.type !== "FINISH_TO_START" || dep.lagWorkdays !== 0)
            continue;
        if (!shiftedCriticalIds.has(dep.predecessorId) || !shiftedCriticalIds.has(dep.successorId))
            continue;
        ...
        if (!predecessor || !successor)
            continue;
        const confidence = Math.min(0.82, predecessor.confidence?.overall ?? 0.5, successor.confidence?.overall ?? 0.5);
        levers.push({
            id: `handoff:${dep.id}`,
            type: "SAME_DAY_HANDOFF_REVIEW",
            predecessorId: dep.predecessorId,
            successorId: dep.successorId,
            earliestRelevantDate: predecessor.finish?.likely ?? successor.start?.likely ?? model.forecastAnchorDate,
            maxRecoverableWorkdays: 1,
            confidence,
            requiresValidation: true,
            action: `Review whether ${predecessor.activityName} can field-release ${successor.activityName} on the same workday once completion/inspection evidence is documented. Do not overlap without PM, trade, and inspection confirmation.`,
        });
    }
    for (const shifted of shiftedCritical) {
        const activity = model.activities[shifted.activityId];
        const forecast = current.activityForecasts?.[shifted.activityId];
        if (!activity || !forecast)
            continue;
        const compressible = Math.max(0, activity.duration.likely - activity.duration.optimistic);
        if (compressible > 0) {
            levers.push({
                id: `duration:${activity.id}`,
                type: "DURATION_COMPRESSION_REVIEW",
                activityId: activity.id,
                earliestRelevantDate: forecast.start?.likely ?? model.forecastAnchorDate,
                maxRecoverableWorkdays: compressible,
                confidence: Math.min(0.8, (forecast.confidence?.overall ?? 0.5) * 0.9),
                requiresValidation: true,
                action: `Confirm with the responsible trade whether ${activity.name} can be completed in the optimistic ${activity.duration.optimistic}-workday duration instead of the likely ${activity.duration.likely}-workday duration without reducing scope, quality, safety, or required inspections.`,
            });
        }
        for (const constraintId of activity.constraintIds ?? []) {
            const constraint = model.constraints[constraintId];
            if (!constraint || !constraint.hard || constraint.state === "SATISFIED")
                continue;
            const req = (forecast.requiredBy ?? []).find((item) => item.kind === "CONSTRAINT" && item.id === constraintId);
            protectionActions.push({
                id: `protect:${constraint.id}`,
                type: "EARLY_CONSTRAINT_CLOSEOUT",
                activityId: activity.id,
                constraintId: constraint.id,
                requiredBy: req?.requiredBy ?? forecast.start?.likely ?? model.forecastAnchorDate,
                truthState: constraint.state,
                action: `Close ${constraint.label} before ${req?.requiredBy ?? forecast.start?.likely ?? model.forecastAnchorDate} so the current delay does not compound into ${activity.name}.`,
            });
        }
    }
    levers.sort((a, b) => a.earliestRelevantDate.localeCompare(b.earliestRelevantDate) || b.maxRecoverableWorkdays - a.maxRecoverableWorkdays || b.confidence - a.confidence);
    protectionActions.sort((a, b) => a.requiredBy.localeCompare(b.requiredBy));
    const bestCapacity = levers.reduce((max, lever) => Math.max(max, lever.maxRecoverableWorkdays ?? 0), 0);
    const recoverableWorkdays = Math.min(delayWorkdays, bestCapacity);
    const recoveryAvailable = recoverableWorkdays > 0;
    const topRecoveryLevers = levers.filter((lever) => (lever.maxRecoverableWorkdays ?? 0) > 0).slice(0, 6);
    const recoveryConfidence = recoveryAvailable && topRecoveryLevers.length
        ? Math.max(0, Math.min(1, topRecoveryLevers.slice(0, 3).reduce((sum, lever) => sum + lever.confidence, 0) / Math.min(3, topRecoveryLevers.length)))
        : 0;
    const recoverableCompletionLikely = recoveryAvailable && currentCompletionLikely
        ? (0, date_js_1.addWorkdays)(currentCompletionLikely, -recoverableWorkdays, model.calendar)
        : currentCompletionLikely;
    return {
        ...
        protectionActions: protectionActions.slice(0, 8),
        rule: "Recovery options are advisory. Howler does not convert a recovery lever into a schedule commitment until the required trade, field, inspection, or PM evidence is confirmed.",
    };
}
exports.analyzeRecovery = analyzeRecovery;
function requiredByFor(model, activity, likelyStart, likelyScenario) {
    ...
    for (const constraintId of activity.constraintIds) {
        ...
        const leadDate = leadWorkdays > 0 ? (0, date_js_1.addWorkdays)(likelyStart, -leadWorkdays, model.calendar) : likelyStart;
        const requiredBy = c.state === "COMMITTED" && c.readiness?.likely ? c.readiness.likely : leadDate;
        items.push({
            kind: "CONSTRAINT",
            id: c.id,
            label: c.label,
            ...
            truthState: c.state,
            requiredBy,
            action: c.state === "COMMITTED" ? `Verify ${c.label} is actually satisfied by ${requiredBy}` : c.state === "STALE_REVERIFY" ? `Re-verify ${c.label} by ${requiredBy}` : `Resolve/confirm ${c.label} by ${requiredBy}`,
        });
    }
    for (const dep of graph.incoming[activity.id] ?? []) {
...
function generateForecast(model, generatedAt, version, baseline, impactedActivityIds) {
    ...
    const delta = buildForecastDelta(model, activityForecasts, completion, baseline);
    const snapshot = {
        ...
        impactActivityIds: impactedActivityIds ? [...impactedActivityIds] : Object.keys(model.activities),
        activityForecasts,
        pmActions: buildPmActions(model, activityForecasts),
        completion,
        supersededSources: supersededSourceSummary(model),
        ...(delta ? { delta, deltaFromSnapshotId: baseline.id } : {}),
    };
...
<main>
  ...
  <section class="card">
    <div class="buttons">
      ...
      <button id="recovery" class="secondary">v0.9.3 Recovery Review</button>
      <button id="previewEvidence">Preview v0.9.3 Supersession</button>
      <button id="applyEvidence" class="danger">Apply v0.9.3 Shadow Reforecast</button>
...
(() => {
  ...
  async function buildMasonryEvidenceSimulation() {
    ...
    return {
      ...
      note: 'Staging-only v0.9.3 supersession test. Retains the Aug 24 source in history, marks it superseded, and keeps the Aug 27 commitment current. Does not touch live systems.'
    };
  }
  ...
  document.getElementById('previewEvidence').addEventListener('click', async () => {
    buttons.forEach((button) => button.disabled = true);
    show('Building v0.9.3 supersession preview...');
    try {
      ...
      show({ simulation: true, persisted: false, instructions: 'Review supersededSources, PM action dates, recoveryAnalysis, delta, and oversight. Tap Apply v0.9.3 Shadow Reforecast to persist only to staging.', preview });
    } catch (error) {
      show({ error: error instanceof Error ? error.message : String(error) });
    } finally {
      buttons.forEach((button) => button.disabled = false);
  ...
  document.getElementById('applyEvidence').addEventListener('click', async () => {
    ...
    if (!confirm('Persist this v0.9.3 supersession event and reforecast to the staging D1 database? The old Aug 24 source remains in history and no live calendar/dashboard/system will be changed.')) return;
...
function createDeboardSeed() {
    ...
    const activities = {
        ...
        roof_procurement: { id: "roof_procurement", name: "Roof package selection / refresh / release", phase: "Procurement", state: "NOT_STARTED", duration: { optimistic: 1, likely: 2, conservative: 3, sourceIds: ["src-roof-action", "src-material-quotes"] }, constraintIds: ["roof-action", "roof-vendor"], sourceIds: ["src-roof-action", "src-material-quotes"] },
    ...
    const constraints = {
        ...
        "engineering-reconcile": { id: "engineering-reconcile", activityId: "structural_reconcile", type: "INFORMATION", label: "Plan/LVL/post reconciliation status", state: "STALE_REVERIFY", hard: true, readiness: w("2026-08-26", "2026-08-27", "2026-08-28"), sourceIds: ["src-plans", "src-engineering", "src-dashboard"], verification: "STALE_REVERIFY" },
        ...
        "framing-material": { id: "framing-material", activityId: "framing", type: "MATERIAL", label: "Walls/subfloor/LVL framing package received, counted, undamaged, staged", state: "UNVERIFIED", hard: true, readiness: w("2026-08-28", "2026-08-28", "2026-08-31"), sourceIds: ["src-building-package", "src-engineering"], verification: "UNVERIFIED" },
        "roof-action": { id: "roof-action", activityId: "roof_procurement", type: "INFORMATION", label: "Roof vendor analysis / contact Tony on Aug 26", state: "COMMITTED", hard: false, readiness: w("2026-08-26", "2026-08-26", "2026-08-26"), sourceIds: ["src-roof-action"], verification: "PM_CONFIRMED" },
        "roof-vendor": { id: "roof-vendor", activityId: "roof_procurement", type: "INFORMATION", label: "Roof vendor/package decision and release", state: "UNVERIFIED", hard: true, readiness: w("2026-08-26", "2026-08-27", "2026-08-28"), sourceIds: ["src-roof-action", "src-material-quotes"], verification: "UNVERIFIED" },
        ...
        "electrical-trade": { id: "electrical-trade", activityId: "electrical_rough", type: "TRADE_AVAILABILITY", label: "Jason Bonham electrical rough-in mobilization", state: "FORECASTED", hard: true, sourceIds: ["src-trades"], verification: "CORROBORATED" },
        "plumbing-trade": { id: "plumbing-trade", activityId: "plumbing_rough", type: "TRADE_AVAILABILITY", label: "Medina plumbing rough-in mobilization", state: "FORECASTED", hard: true, sourceIds: ["src-trades", "src-plumbing"], verification: "CORROBORATED" },
        "hvac-trade": { id: "hvac-trade", activityId: "hvac_rough", type: "TRADE_AVAILABILITY", label: "Anthony Parker HVAC rough-in mobilization", state: "FORECASTED", hard: true, sourceIds: ["src-trades", "src-hvac"], verification: "CORROBORATED" },
        ...
        "paint-trade": { id: "paint-trade", activityId: "prime_paint", type: "TRADE_AVAILABILITY", label: "Dylan Jasper painting mobilization", state: "FORECASTED", hard: true, sourceIds: ["src-trades", "src-budget"], verification: "CORROBORATED" },
        ...
        "flooring-trade": { id: "flooring-trade", activityId: "flooring", type: "TRADE_AVAILABILITY", label: "Flooring labor mobilization", state: "FORECASTED", hard: true, sourceIds: ["src-budget"], verification: "CORROBORATED" },
...
async function reviewedRun(repo, projectId, event) {
    ...
    if (!event || typeof event !== "object")
        throw new http_js_1.HttpError(400, "Event body is required");
    if (!Number.isInteger(event.baseRevision) || event.baseRevision !== model.revision)
        throw new http_js_1.HttpError(409, `Event baseRevision must equal current project revision ${model.revision}`);
    if (!Array.isArray(event.sourceIds) || !Array.isArray(event.impactSeedActivityIds) || !Array.isArray(event.mutations))
        throw new http_js_1.HttpError(400, "Event sourceIds, impactSeedActivityIds, and mutations must be arrays");
    if (!Number.isFinite(Date.parse(event.occurredAt)) || !Number.isFinite(Date.parse(event.receivedAt)))
        throw new http_js_1.HttpError(400, "Event occurredAt and receivedAt must be valid ISO timestamps");
    const comparisonBaseline = latest ?? baseline;
    const run = (0, engine_js_1.forecastAfterEvent)(model, event, event.receivedAt, nextVersion(latest), comparisonBaseline);
...
async function handle(request, env) {
    ...
    if (request.method === "POST" && parts.join("/") === "v1/projects/deboard-v091/seed") {
        if (await repo.projectExists("deboard-v091"))
            throw new http_js_1.HttpError(409, "DeBoard v0.9.1 is already seeded");
    ...
    if (request.method === "GET" && parts.length === 5 && parts[3] === "forecast" && parts[4] === "recovery") {
        const model = await repo.loadProject(projectId);
        if (!model)
            throw new http_js_1.HttpError(404, `Project ${projectId} not found`);
        const latest = await repo.loadLatestForecast(projectId);
        if (!latest)
            throw new http_js_1.HttpError(404, `No forecast exists for ${projectId}`);
        ...
        const recovery = (0, solver_js_1.analyzeRecovery)(model, latest, baseline);
        return (0, http_js_1.json)({
            projectId,
            projectRevision: model.revision,
            ...
            recovery,
            publicationGate: { forecastAllowed: true, commitmentEligible: false, publishable: false, mode: env.HOWLER_MODE ?? "shadow" },
            stagingOnly: true,
        });
    }
    ...
    if (request.method === "POST" && parts.length === 5 && parts[3] === "events" && parts[4] === "preview") {
        ...
        return (0, http_js_1.json)({
            ...
            recoveryAnalysis: result.run.candidate.recoveryAnalysis ?? null,
            ...
            publishable: (env.HOWLER_MODE ?? "shadow") === "controlled" && result.run.publishable,
            reviewToken: result.reviewToken,
            persisted: false,
            mode: env.HOWLER_MODE ?? "shadow",
            stagingOnly: (env.HOWLER_MODE ?? "shadow") === "shadow",
        });
    }
    if (request.method === "POST" && parts.length === 5 && parts[3] === "events" && parts[4] === "apply-shadow") {
        if ((env.HOWLER_MODE ?? "shadow") !== "shadow") {
            throw new http_js_1.HttpError(403, "Shadow evidence application is available only while HOWLER_MODE=shadow");
        }
        ...
        if (result.reviewToken !== body.reviewToken) {
            throw new http_js_1.HttpError(409, "Preview no longer matches the current project state. Re-preview before applying shadow evidence.");
        }
...
class D1HowlerRepository {
    ...
    async loadForecastById(projectId, snapshotId) {
        const row = await this.db.prepare(`SELECT snapshot_json AS json FROM forecast_snapshots
      WHERE project_id = ? AND snapshot_id = ? LIMIT 1`)
            .bind(projectId, snapshotId)
            .first();
        return row ? parseJson(row.json, `forecast ${snapshotId}`) : undefined;
    }
    ...
    async commitShadowTransition(transition) {
        const { expectedRevision, modelAfterEvent, event, candidate, oversight } = transition;
        if (!candidate || candidate.status === "PUBLISHED")
            throw new Error("Shadow transition requires a non-published forecast candidate");
        if (modelAfterEvent.revision !== expectedRevision + 1)
            throw new Error("Shadow transition revision increment is invalid");
        if (event.baseRevision !== expectedRevision)
            throw new Error("Shadow event baseRevision does not match expectedRevision");
        if (candidate.modelRevision !== modelAfterEvent.revision)
            throw new Error("Shadow candidate modelRevision does not match event-applied project revision");
        if (oversight.candidateSnapshotId !== candidate.id)
            throw new Error("Shadow oversight review does not reference candidate snapshot");
        const statements = [
            this.db.prepare(`INSERT INTO project_events
        (project_id, event_id, base_revision, new_revision, event_type, occurred_at, received_at, event_json, model_after_json)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        ...
        (snapshot_id, project_id, model_revision, version, status, snapshot_json, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)`)
                .bind(candidate.id, candidate.projectId, candidate.modelRevision, candidate.version, candidate.status, JSON.stringify(candidate), candidate.generatedAt),
            this.db.prepare(`INSERT INTO oversight_reviews
        (review_id, project_id, candidate_snapshot_id, decision, review_json, created_at)
        VALUES (?, ?, ?, ?, ?, ?)`)
                .bind(oversight.id, oversight.projectId, oversight.candidateSnapshotId, oversight.decision, JSON.stringify(oversight), oversight.createdAt),
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            if (message.includes("HOWLER_REVISION_CONFLICT") || message.includes("UNIQUE constraint failed: project_events.project_id, project_events.new_revision")) {
                throw new storage_js_1.RevisionConflictError(`Project ${event.projectId} changed before this shadow update could be applied`);
            }
            throw error;
        }
    }
