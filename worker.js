// HOWLER Scheduling Intelligence staging bundle v0.9.4
...
function recoveryUrgency(requiredBy, anchorDate) {
    if (!requiredBy)
        return "UNSCHEDULED";
    if (requiredBy < anchorDate)
        return "OVERDUE";
    if (requiredBy === anchorDate)
        return "DUE_NOW";
    return "UPCOMING";
}
function buildProtectionActions(model, current) {
    const actions = [];
    const seen = new Set();
    for (const forecast of Object.values(current.activityForecasts ?? {})) {
        ...
        for (const constraintId of activity.constraintIds ?? []) {
            ...
            const requiredBy = req?.requiredBy ?? forecast.start?.likely ?? model.forecastAnchorDate;
            const key = `${forecast.activityId}:${constraint.id}`;
            if (seen.has(key))
                continue;
            seen.add(key);
            const severity = constraint.state === "BLOCKED" ? "BLOCK" : constraint.state === "STALE_REVERIFY" ? "CRITICAL" : "WATCH";
            actions.push({
                ...
                severity,
                critical: true,
                confidence: forecast.confidence?.overall ?? 0.5,
                action: `Close ${constraint.label} by ${requiredBy} to protect ${activity.name} and prevent a downstream critical-path shift.`,
            });
        }
    }
    const severityRank = { BLOCK: 0, CRITICAL: 1, WATCH: 2 };
    const urgencyRank = { OVERDUE: 0, DUE_NOW: 1, UPCOMING: 2, UNSCHEDULED: 3 };
...
function buildStandbyRecoveryLevers(model, current) {
    const graph = (0, graph_js_1.buildGraphIndex)(model);
    const levers = [];
    const criticalIds = new Set(Object.values(current.activityForecasts ?? {}).filter((forecast) => forecast.critical).map((forecast) => forecast.activityId));
    for (const dep of Object.values(model.dependencies)) {
    ...
    for (const activityId of graph.topologicalOrder) {
        ...
        levers.push({
            ...
            requiresValidation: true,
            standby: true,
            action: `Pre-qualify whether ${activity.name} can be completed in the optimistic ${activity.duration.optimistic}-workday duration instead of the likely ${activity.duration.likely}-workday duration without reducing scope, quality, safety, or required inspections.`,
        });
    }
...
function analyzeRecovery(model, current, baseline) {
    ...
    const nextRiskDate = protectionActions.length ? protectionActions[0].requiredBy : null;
    const blockedProtectionCount = protectionActions.filter((item) => item.truthState === "BLOCKED").length;
    const criticalExposureCount = protectionActions.length;
    const standbyRecoveryCapacityWorkdays = standbyLevers.reduce((max, lever) => Math.max(max, lever.maxRecoverableWorkdays ?? 0), 0);
    const standbyConfidence = standbyLevers.length
        ? Math.max(0, Math.min(1, standbyLevers.slice(0, 3).reduce((sum, lever) => sum + lever.confidence, 0) / Math.min(3, standbyLevers.length)))
        : 0;
    if (delayWorkdays <= 0) {
        return {
    ...
    for (const standby of standbyLevers) {
        const touchesShifted = standby.activityId
            ? shiftedCriticalIds.has(standby.activityId)
            : shiftedCriticalIds.has(standby.predecessorId) || shiftedCriticalIds.has(standby.successorId);
        if (!touchesShifted)
            continue;
    ...
    if (request.method === "GET" && parts.length === 5 && parts[3] === "forecast" && parts[4] === "recovery") {
        ...
        return (0, http_js_1.json)({
            ...
            recoveryLayer: {
                ...
                standbyRecoveryCapacityWorkdays: recovery.standbyRecoveryCapacityWorkdays ?? 0,
            },
