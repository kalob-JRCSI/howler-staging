// HOWLER Scheduling Intelligence staging bundle v0.9.2
...
<main>
  ...
  <section class="card">
    <div class="buttons">
      ...
      <button id="previewEvidence">Preview v0.9.2 Evidence Loop</button>
      <button id="applyEvidence" class="danger">Apply Shadow Reforecast</button>
...
(() => {
  ...
  async function buildMasonryEvidenceSimulation() {
    ...
    const sourceId = 'src-v092-field-sim-' + Date.now();
    return {
      id: 'deboard-v092-field-sim-' + Date.now(),
      ...
      mutations: [
        { op: 'UPSERT_SOURCE', source: { id: sourceId, type: 'FIELD_REPORT', label: 'v0.9.2 staging simulation - CMU verified and masonry start committed', observedAt: now, authority: 0.95, reliability: 0.95 } },
      ...
      payload: { simulation: true, materialVerified: true, masonryStartCommitment: '2026-08-27' },
      note: 'Staging-only v0.9.2 evidence ingestion and dependency reforecast test. Does not touch live systems.'
    };
  }
  ...
  document.getElementById('previewEvidence').addEventListener('click', async () => {
    buttons.forEach((button) => button.disabled = true);
    show('Building v0.9.2 field-evidence preview...');
    try {
      ...
      show({ simulation: true, persisted: false, instructions: 'Review dateBasis, assumptions, impactActivityIds, delta, and oversight. Tap Apply Shadow Reforecast to persist only to staging.', preview });
    } catch (error) {
      show({ error: error instanceof Error ? error.message : String(error) });
    } finally {
      buttons.forEach((button) => button.disabled = false);
  ...
  document.getElementById('applyEvidence').addEventListener('click', async () => {
    ...
    if (!confirm('Persist this simulated evidence event and its reforecast to the staging D1 database? No live calendar/dashboard/system will be changed.')) return;
